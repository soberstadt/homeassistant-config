...
class RokuDevice(MediaPlayerDevice):
    ...
    def play_media(self, media_type, media_id, **kwargs):
        _LOGGER.warning("Media Type: %s, Media ID: %s", media_type, media_id)

        channel = self.roku[media_type]

        if channel is None:
            return None

        if channel.id == 'tvinput.dtv':
            self.roku._post('/launch/tvinput.dtv', params={'ch': media_id})
        else:
            self.roku.launch(channel, { 'contentId': media_id })
